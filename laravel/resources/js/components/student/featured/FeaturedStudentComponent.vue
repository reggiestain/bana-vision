<template>
  ...
  <div class="card-header">
      ...
      <div class="media-body" >
        <h5 style="font-size: 0.755rem;margin-bottom: 0.15rem;">
          <a class="navbar-logo  ml-auto" href="route('SchoolProfilePage',['userIdNo'=>$post->shared_user_slug])" style="padding-right:6px">
            {{chunk.shared_user_name}} 
            <span style="color:#000000">
            shared a featured student
          </span>
          </a>
        </h5>
        <a style="color: black" >
    ...
    <!--============================SHARED POST OPTIONS DROPDOWN========================================-->
     <div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
        <div v-if="isRouteFeaturedPage"><!--is route featured-->
        <a class="dropdown-item" href="route('ourNeeds',['user'=>$post->user])">
        more needs from 
      </a>
        </div><!--end is route-->
  ...
  <img class="card-img-top show-photo"  :src="getFeaturedPic(chunk)" alt="Schools brand picture" style="background-color:beige;max-height: 30rem; " v-on:click="showphoto(chunk,0)">
    <!--============================CARD BODY===============================-->
  <div class="card-body" style="padding: 0"> 
      ...
      <div class="col-3 p-3" style="    text-align: center;">
        <span class="fa-stack fa-2x">
          <i class="fas fa-user-graduate fa-stack-2x" style="color:#4169e1"></i>
        </span>
      </div>
      <!--==============================EVENT DETAILS==============================-->
      <div class="col-9 p-3" >
        <h5>
          <strong class="text-dark">
            Our featured student : {{chunk.school_student.name}} {{chunk.school_student.surname}}
          </strong>
        </h5>
            <p class="text-muted">
              {{chunk.achievements}}
...
<span class="p-2 pb-0">
  <comments :post_id="chunk.id" :key_="chunkIndex" post_type="OurEvent" ></comments>
...
<script>
	export default {
		props:['post','is_route_featured_page'],
		data: function() {
	 		return {
		...
		methods:
		{
  			...
  			profile_pic:function(user_id)
			{
              ...
              showphoto:function(featured_student,active)
    {
      console.log('%c'+'im clicked','font-size:140px');
      console.log(featured_student);
      //console.log($(event.target).attr('data-event'));
      //var featured_student = $(event.target).attr('data-event');
      //featured_student.images
      var imgsrc = $(event.target).attr('src');
      //var studentsImages = document.getElementById("student-pictures");
      var mrkup='';
      for (var i = 1; i < featured_student.school_student.images.length; i++) 
      {
        mrkup += '<div class="carousel-item">'+
      '<img class="img-fluid d-block mx-auto modal-img"  src="/post-picture/'+featured_student.school_student.images[i].name+'/events/null/null" alt="Third slide">'+
    '</div>';
      }
      //studentsImages.innerHTML = '';

     var innerHTML = '<div class="carousel-item active"><img class="img-fluid d-block mx-auto" src="'+this.getFeaturedPic(featured_student)+'" id="first-slide-'+featured_student.id+'" alt="First slide" ></div>'+mrkup;
      //$('#first-slide').prop('src',$(event.target).attr('src'));
      var posttext='<h4>Our featured student : '+featured_student.school_student.name+' '+featured_student.school_student.surnname+'</h4><ul>'+
      '<li ><strong>Achievements:</strong> '+featured_student.achievements+'</li>'+
      '</ul>';
      var modal_object = 
      {
        'id':featured_student.id,
        'auth_id':this.auth_id,
        'post_type':"OurEvent",
        'innerhtml':innerHTML,
        'posttext':posttext,
        'posterimg':this.profile_pic(featured_student.school.user[0].id),
        'postername':featured_student.school.user[0].name
      };
      var reff = 'postpic'+featured_student.id;
      this.ref_=reff;
      this.$store.commit('setpicid',featured_student.id);
      console.log(this.$store.state.pic_post_id);
      //$('#PostPicCarousel-modal-'+featured_student.id).modal();
      this.$root.$refs['getpostpic'].launchModal(modal_object); //calls launchmodal method from postpic component
        ...
        {
            var that = this;
            if(this.is_route_featured_page)
            {
