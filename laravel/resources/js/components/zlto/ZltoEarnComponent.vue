<template>
  <div class="earn">
    <div class="top-details row">
      <div class="col-8 info-image"></div>
      <div class="col-4 info-details"></div>
    </div>
    <div class="row earn">
      <link
        rel="stylesheet"
        href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.14.0/css/all.min.css"
        integrity="sha512-1PKOgIY59xJ8Co8+NE6FZ+LOAZKjy+KY8iq0G4B3CyeY6wYHN3yt9PW0XpSriVlkMXe40PTKnXrLnZ9+fkDaog=="
        crossorigin="anonymous"
      />
      <div class="container">
        <img
          src="data:image/png;base64,"
          alt="Pancake"
        />
        <div class="container-text">
          <h1>Google Training</h1>
          <!-- <div class="container-text-star">
            <span class="fa fa-star checked"></span>
            <span class="fa fa-star checked"></span>
            <span class="fa fa-star checked"></span>
            <span class="fa fa-star checked"></span>
            <span class="fa fa-star checked"></span>
          </div> -->
          <p>
            If you would like to recieve traing on how google products work.
            This is the best chance to get to know each product and how it works
            .
          </p>
          <div class="container-text-timing">
            <div class="container-text-timing_time">
              <h2>Time to Complete</h2>
              <p>30 min</p>
            </div>
            <div class="container-text-timing_time">
              <h2>Type</h2>
              <p>Questionnaire</p>
            </div>
            <div class="container-text-timing_time">
              <h2>Zlto</h2>
              <p>40</p>
            </div>
          </div>
          <button class="btn">
            START TASK <i class="fa fa-arrow-right"></i>
          </button>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {};
</script>

<style scoped>
.spend {
  font-family: Arial, Helvetica, sans-serif;
  display: grid;
  place-items: center;
  min-height: 100vh;
}
.container {
  background-color: #fff;
  position: relative;
  display: grid;
  grid-template-columns: 150px 600px;
  border-radius: 5px;
  box-shadow: 0 15px 20px rgba(0, 0, 0, 0.356);
}
.container img {
  object-fit: cover;
  width: 150;
  margin-top: auto;
  margin-bottom: auto;
  height: 150px;
  border-radius: 5px 0 0 5px;
}
.container .btn {
  position: absolute;
  bottom: -20px;
  right: -20px;
  border: none;
  outline: none;
  display: flex;
  align-items: center;
  background-color: #fc9400;
  color: #fff;
  padding: 22px 45px;
  font-size: 1rem;
  text-transform: uppercase;
  border-radius: 5px;
  cursor: pointer;
  box-shadow: 0 10px 20px rgba(0, 0, 0, 0.294);
}
.container .btn i {
  margin-left: 20px;
  font-size: 1.5rem;
}
.container-text {
  padding: 40px 40px 0;
}
.container-text h1 {
  color: #478ac9;
  font-weight: 400;
}
.container-text .container-text-star span {
  font-size: 0.6rem;
  color: #ffa800;
  margin: -5px 0 20px;
}
.container-text p {
  font-size: 0.7rem;
}
.container-text .container-text-timing {
  display: flex;
  margin: 20px 0 10px;
}
.container-text .container-text-timing .container-text-timing_time {
  margin-right: 40px;
}
.container-text .container-text-timing h2 {
  margin-bottom: 5px;
  font-size: 0.8rem;
  font-weight: 400;
  color: #818189;
}
.container-text .container-text-timing p {
  color: #478ac9;
  font-weight: bold;
  font-size: 1rem;
}
</style>
